    ...
    <div className={`p-6 rounded-2xl shadow-lg mb-6 ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white'
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={() => onDaySelect(Math.min(weekPlan.length - 1, selectedDay + 1))}
        ...
        {weekPlan.map((day, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className={`text-lg font-bold mb-1 ${isSelected ? 'text-white' : isDark ? 'text-white' : 'text-gray-900'
                }`}>
                {new Date(day.date).getDate()}
              </div>
              {isToday && !isSelected && (
                <div className={`w-1.5 h-1.5 rounded-full mx-auto ${isDark ? 'bg-emerald-400' : 'bg-emerald-600'
                  }`}></div>
              )}
              <div className={`text-xs mt-1 ${isSelected ? 'text-white/80' : isDark ? 'text-gray-500' : 'text-gray-600'
                }`}>
      ...
      {/* Daily Summary */}
      <div className={`mt-4 p-4 rounded-xl ${isDark ? 'bg-gray-700/50' : 'bg-gray-50'
        }`}>
